...
class totara_notification_webapi_middleware_validate_event_class_name_testcase extends advanced_testcase {
     ...
     * @return void
     */
    public function test_validate_invalid_notifiable_event(): void {
        $ec = execution_context::create('dev');
        $payload = new payload(['event_class' => 'dota_2'], $ec);

        $middleware = new validate_event_class_name('event_class', true);
     ...
     * @return void
     */
    public function test_validate_event_without_existing_in_payload(): void {
        $ec = execution_context::create('dev');
        $payload = new payload(['ccc' => 'ddd'], $ec);

        $middleware = new validate_event_class_name('ddd', true);
     ...
     * @return void
     */
    public function test_validate_event_without_existing_in_payload_but_not_required(): void {
        $ec = execution_context::create('dev');
        $payload = new payload(['ccc' => 'dd'], $ec);

        $middleware = new validate_event_class_name('ddd');
     ...
     * @return void
     */
    public function test_validate_notifiable_event(): void {
        ...
        $ec = execution_context::create('dev');
        $payload = new payload(
            ['event_name' => mock_event::class],
            $ec
        );

        $middleware = new validate_event_class_name('event_name', true);
        $result = $middleware->handle(
            $payload,
            function (payload $payload): result {
                return new result($payload->get_variables());
