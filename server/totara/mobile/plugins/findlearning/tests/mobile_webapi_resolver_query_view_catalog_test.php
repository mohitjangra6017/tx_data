...
class mobile_findlearning_webapi_resolver_query_view_catalog_testcase extends advanced_testcase {
    ...
    public function test_embedded_query() {
        ...
        try {
            ...
            $this->assertNotEmpty($data['catalog_page']);
            $page = $data['catalog_page'];
            ...
            foreach ($page['items'] as $item) {
                $expect = array_shift($expected);
                $this->assertSame($expect['type'], $item['item_type']);
                $this->assertSame($expect['name'], $item['title']);
                $this->assertMatchesRegularExpression('|^https://www\.example\.com/.*|', $item['mobile_image']);
            }

            $this->assertEquals('16', $page['max_count']); // The total number of unchecked records.
            $this->assertEquals('14', $page['pointer']); // The number of checked records.
            $this->assertFalse($page['final_page']); // Whether this page is the last.
