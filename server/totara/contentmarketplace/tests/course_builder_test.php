...
class totara_contentmarketplace_course_builder_testcase extends testcase {
     ...
     * @return void
     */
    public function test_create_course_without_enabling_self_enrolment(): void {
        $admin = get_admin();
        $generator = generator::instance();
        ...
        $not_enabled_enrols = ['self', 'guest'];
        $enrol_repository = enrol::repository();

        foreach ($not_enabled_enrols as $enrol_name) {
            $enrol = $enrol_repository->find_enrol($enrol_name, $course_id);
            self::assertTrue($enrol->is_disabled());
            self::assertFalse($enrol->is_enabled());
        }
    }
     ...
     * @return void
     */
    public function test_create_course_with_enrolment_enabled_by_default(): void {
        $admin = get_admin();
        $generator = generator::instance();
        ...
        $enrol_repository = enrol::repository();
        $self_enrol = $enrol_repository->find_enrol('self', $course_id, true);

        self::assertTrue($self_enrol->is_enabled());
        self::assertFalse($self_enrol->is_disabled());

        $guest_enrol = $enrol_repository->find_enrol('guest', $course_id, true);
        self::assertFalse($guest_enrol->is_enabled());
        self::assertTrue($guest_enrol->is_disabled());
    }

    /**
     * @return void
     */
    public function test_create_course_with_enrolment_enabled_by_default_and_guest_enrol(): void {
        $admin = get_admin();
        $generator = generator::instance();
        ...
        $guest_enrol = $enrol_repository->find_enrol('guest', $course_id, true);
        self::assertTrue($guest_enrol->is_enabled());
        self::assertFalse($guest_enrol->is_disabled());
    }

    /**
     * @return void
     */
    public function test_create_course_with_invalid_enrolment_enable(): void {
        $admin = get_admin();
        $generator = generator::instance();
        ...
        $enrol_repository = enrol::repository();
        $enrol = $enrol_repository->find_enrol('g2a', $course_id);

        self::assertNull($enrol);
    }

    /**
     * @return void
     */
    public function test_create_course_with_facetoface_enrol(): void {
        global $CFG;

        // Add facetoface to the enrolment enabled.
        $enrol_enabled = explode(',', $CFG->enrol_plugins_enabled);
        $enrol_enabled[] = 'totara_facetoface';

        set_config('enrol_plugins_enabled', implode(',', $enrol_enabled));
        ...
        $enrol_repository = enrol::repository();
        $enrol = $enrol_repository->find_enrol('totara_facetoface', $course_id, true);
