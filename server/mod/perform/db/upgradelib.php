 ...
 */
function mod_perform_maintain_active_date_picker_year_ranges() {
    global $DB;

    $active_date_picker_elements = $DB->get_recordset_sql("
        SELECT element.id, element.data
        FROM {perform_element} element
        JOIN {perform_section_element} section_element ON section_element.element_id = element.id
        JOIN {perform_section} section ON section.id = section_element.section_id
        JOIN {perform} activity on activity.id = section.activity_id
        WHERE element.plugin_name = 'date_picker'
    ...
    foreach ($active_date_picker_elements as $date_picker_element) {
        ...
        $DB->set_field('perform_element', 'data', json_encode($new_value, JSON_THROW_ON_ERROR), [
