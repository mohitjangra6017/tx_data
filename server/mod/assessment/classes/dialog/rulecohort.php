...
class rulecohort extends totara_dialog_content
{
    ...
    {
        global $DB;
        return $DB->get_records('cohort', [], 'name', 'id, name AS fullname');
    }

    public function populate_selected_items_pane($elements): string
    {
        $operatormenu = array();
        $operatormenu[rule::OP_IN_EQUAL] = get_string('incohort', 'totara_cohort');
        $operatormenu[rule::OP_IN_NOTEQUAL] = get_string('notincohort', 'totara_cohort');
