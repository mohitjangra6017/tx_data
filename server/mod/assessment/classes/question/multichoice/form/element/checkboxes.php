...
class checkboxes extends \totara_form\form\element\checkboxes implements question_element
{
    ...
    public function export_for_template_role(renderer_base $output, role $questionrole): array
    {
        ...
        $context['question_template'] = 'assquestion_multichoice/element_checkboxes';
        return $context;
    }
    ...
    ): array
    {
        ...
        foreach ($question->get_responses() as $i => $response) {
            ...
            $checked = in_array($response->text, (array)$currentValue);
