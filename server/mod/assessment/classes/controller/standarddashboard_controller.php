 ...
 *
 * DEVIOTIS2
 * - This Dashboard shows only incomplete assessments
...
class standarddashboard_controller extends dashboard_controller
{
    ...
    {
        return 'mod/assessment:viewdashboard';
    }

    public function __construct(moodle_page $page, moodle_database $db)
    {

        $this->reportshortname = 'assessment_dashboard';
        $this->title = get_string('navigation:dashboard', $this->plugin);

        parent::__construct($page, $db);

        $this->page->set_url(new moodle_url('/mod/assessment/dashboard.php', ['type' => 'standard']));
    }
