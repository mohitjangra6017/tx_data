...
class completeddashboard_controller extends dashboard_controller
{
    ...
    {
        return 'mod/assessment:viewcompleteddashboard';
    }

    public function __construct(moodle_page $page, moodle_database $db)
    {

        $this->reportshortname = 'assessment_dashboardcompleted';
        $this->title = get_string('navigation:completeddashboard', $this->plugin);

        parent::__construct($page, $db);

        $this->page->set_url(new moodle_url('/mod/assessment/dashboard.php', ['type' => 'completed']));
    }
