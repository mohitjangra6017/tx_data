...
class rb_assessment_dashboard_embedded extends rb_base_embedded
{
    ...
    {
        $this->url = '/mod/assessment/dashboard.php?type=standard';
        $this->source = 'assessment_dashboard';
        $this->shortname = 'assessment_dashboard';
        $this->fullname = get_string('dashboard', 'rb_source_assessment');
        ...
        $this->columns = [
            ...
            ['type' => 'assessmentcompl', 'value' => 'status', 'heading' => get_string('activitystatus', $this->plugin)],
            ['type' => 'assessment', 'value' => 'viewsummary', 'heading' => get_string('viewsummary', $this->plugin)],
        ];

        $this->embeddedparams = [];
        $this->embeddedparams['attemptincomplete'] = 1; // Filter by this status.

        $this->filters = []; // No filtering necessary now we have tabbed reports.
        ...
        if (!has_capability('mod/assessment:viewdashboard', context_system::instance())) {
    ...
    {
        return has_capability('mod/assessment:viewdashboard', context_system::instance())
