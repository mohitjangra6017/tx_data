 ...
 */
function hvp_update_grades($hvp, $userid=0, $nullifnone=true) {
    $grades = hvp_get_user_grades($hvp, $userid);
    hvp_grade_item_update($hvp, $grades);
}

function hvp_get_user_grades($hvp, $userid=0) {
    global $CFG, $DB;
    require_once($CFG->libdir . '/gradelib.php');
    $params = array('content_id' => $hvp->id);
    if ($userid) {
    ...
    $records = $DB->get_records('hvp_xapi_results', $params, '', 'user_id,raw_score,max_score');
    $grades = array();
    foreach ($records as $record) {
        $grade = ($record->raw_score / $record->max_score) * $gradeitem->grademax;
        $grades[$record->user_id] = (object) array(
            'rawgrade' => $grade,
            'finalgrade' => $grade,
