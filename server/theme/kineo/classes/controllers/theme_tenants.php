...
class theme_tenants extends admin_controller {
    ...
    public function action(): tui_view {
        ...
        if (empty($CFG->tenantsenabled)) {
            $url = new moodle_url("/theme/kineo/theme_settings.php", ['theme_name' => $this->theme]);
            redirect($url->out());
        }

        if (!empty($USER->tenantid)) {
            $url = new moodle_url("/theme/kineo/theme_settings.php", ['theme_name' => $this->theme, 'tenant_id' => $USER->tenantid]);
            redirect($url->out());
        }
        ...
        $tui_view = tui_view::create('theme_kineo/pages/ThemeTenants', $props);
