        ...
        if ($article['image']) {
            $image = '
                <div class="' . ($side ? 'ap-article-image-container-side' : 'ap-article-image-container-top') . '">
                    <div class="' . ($side ? 'ap-article-image-container-side-inner' : 'ap-article-image-container-top-inner') . '">
                         <img src="'.$article["image"].'" alt="" class="ap-article-image" />
                    </div>
                </div>';
        }
            ...
            <div class='ap-article'>
              <a class='ap-article-link' href='{$article['url']}' title='" . htmlspecialchars($article['title'], ENT_QUOTES) . "' target='_blank'>
                  ...
                  <div class='" . (($side && $article['image']) ? 'ap-margin-right' : '') . "'>
                      <div class='ap-article-title'>". html_entity_decode(htmlspecialchars($title)) . "</div>
    ...
    function get_content() {
        ...
        if ($this->config->column === 1) {
            ...
            $this->content->text =
                ...
                '</div>';
        } else if ($this->config->column === 3) {
            $this->content->text =
                '<div class="ap-columns">' .
                implode("\n", array_map(function($item) {
                    return '<div class="ap-three-column">' . $item . '</div>';
                }, $articlehtml)) .
                '</div>';
        }
