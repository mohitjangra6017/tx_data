 ...
 */
global $COURSE, $DB, $PAGE, $CFG, $USER, $SITE, $OUTPUT;

require_once( '../../config.php' );
require_once($CFG->dirroot.'/blocks/rate_course/forms/review_course.php');
...
$hook = new \block_rate_course\hook\rate($course->id);
$hook->execute();
require_login($course->id);

$strtitle = get_string('giverating', 'block_rate_course');
...
$mform = new block_rate_course_review(null, array('courseid' => $course->id));

$existing = $block->check_already_rated($USER->id, $course->id);

if ($mform->is_cancelled() or $existing) {
    redirect(new moodle_url('/course/view.php?id='.$course->id));
} else if ($fromform = $mform->get_data()) {
    $block->save_rating($fromform);
    redirect(new moodle_url('/course/view.php?id='.$course->id));
} else if (!empty($external)) {
    $data = new stdClass();
    $data->userid = $USER->id;
    $data->courseid = $course->id;
    $data->rating = required_param('rating', PARAM_INT);
    $data->review = required_param('review', PARAM_TEXT);
    $block->save_rating($data);
    redirect(new moodle_url('/course/view.php?id='.$course->id));
}
