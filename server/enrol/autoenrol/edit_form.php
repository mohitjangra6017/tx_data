...
class enrol_autoenrol_edit_form extends moodleform {
    ...
    protected function add_general_section($instance, $plugin, $context) {
        ...
        $options = array(enrol_autoenrol_plugin::ENROL_AUTO_COURSE_VIEWED => get_string('courseview', 'enrol_autoenrol'),
        ...
        $this->_form->addElement('select', 'customint2', get_string('enrolon', 'enrol_autoenrol'), $options);
        $this->_form->addHelpButton('customint2', 'enrolon', 'enrol_autoenrol');
        $this->_form->setDefault('customint2', $plugin->get_config('enrolon'));

        $mods = \enrol_autoenrol\helper::get_mods_with_viewed_event();
        $modgroup = [];
        foreach ($mods as $modname) {
            $modgroup[] = $this->_form->createElement('checkbox', $modname, '', get_string('pluginname', "mod_{$modname}"));
        }
        $this->_form->addGroup($modgroup, 'customtext2', get_string('modviewmods', 'enrol_autoenrol'), '<br>');
        $this->_form->disabledIf('customtext2', 'customint2', 'neq', enrol_autoenrol_plugin::ENROL_AUTO_MOD_VIEWED);
        ...
        $this->_form->setType('customtext1', PARAM_NOTAGS);
    ...
    public function validation($data, $files) {
        ...
        $start_date = $data['enrolstartdate'];
        $end_date = $data['enrolenddate'];

        if (!empty($start_date) && !empty($end_date) && $start_date >= $end_date) {
            $errors['enrolenddate'] = get_string('error:enroldate', 'enrol_autoenrol');
        }
