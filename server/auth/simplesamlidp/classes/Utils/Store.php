...
class Store extends \SimpleSAML\Store
{
    ...
    public function get($type, $key)
    {
        ...
        $query = '
            SELECT id, value
              FROM {auth_simplesamlidp_kvstore}
             WHERE type = :type
               AND k = :k
               AND (expire IS NULL
                   OR expire > :now
    ...
    public function set($type, $key, $value, $expire = null)
    {
        ...
        $record = $DB->get_record('auth_simplesamlidp_kvstore', $find);
        if ($record) {
            $data['id'] = $record->id;
            $DB->update_record('auth_simplesamlidp_kvstore', $data);
        } else {
            $DB->insert_record('auth_simplesamlidp_kvstore', $data);
        }
    }
    ...
    public function delete($type, $key)
    {
        ...
        $DB->delete_records('auth_simplesamlidp_kvstore', $data);
    }
    ...
    {
        global $DB;
        $sql = 'DELETE FROM {auth_simplesamlidp_kvstore}
                 WHERE expire < :now';
