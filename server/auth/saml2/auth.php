...
class auth_plugin_saml2 extends auth_plugin_base {
    ...
    public $defaults = [
        ...
        'tolower'            => 0,
        'autocreate'         => 0,
        'totarasync'         => 0, // KINEO CCM - GLOTOT-627
        'spmetadatasign'     => true,
        'showidplink'        => true,
        'alterlogout'        => '',
    ...
    public function loginpage_idp_list($wantsurl) {
        ...
        foreach ($this->metadatalist as $metadata) {
            ...
            foreach ($this->metadataentities[$metadata->idpurl] as $idpentityid => $idp) {
                ...
                // KINEO CCM START for ALDO-152: Run name through filter to allow multilang overrides.
                $idpname = format_string($idpname);
                // KINEO CCM END for ALDO-152.
    ...
    public function saml_login() {
        ...
        // KINEO CCM START - GLOTOT-627
        // Set the 'HR Import' flag (totarasync) for new user accounts.
        if ($newuser && $this->config->totarasync) {
            $user->totarasync = 1;
            user_update_user($user, false, false);
        }
        // KINEO CCM END - GLOTOT-627
