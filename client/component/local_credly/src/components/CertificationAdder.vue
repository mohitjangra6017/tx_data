  ...
  >
    <template v-slot:browse-list="{ disabledItems, selectedItems, update }">
      <AdderFilter
        ...
        @clear="updateFilter"
      />
      <div class="certificationAdder__count">
        {{ itemCount() }}
      </div>
      <Table :data="filteredCertifications" :no-items-text="''">
...
import CredlyCertificationsQuery from 'local_credly/graphql/certifications';
...
export default {
  ...
  watch: {
    open() {
      if (this.open) {
        this.$apollo.queries.certifications.refetch().then(() => {
  ...
  apollo: {
    certifications: {
      query: CredlyCertificationsQuery,
      update: (data) => {
        return data.certifications.items;
      },
    }
  },
  ...
  methods: {
    ...
    updateFilter({ searchTerm }) {
      this.filteredCertifications = this.certifications.filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase()));

    },
    itemCount() {
      return this.$str(
        "certification_adder:showing_x_of_y_certifications",
        "local_credly",
        {
          count: this.filteredCertifications.length,
