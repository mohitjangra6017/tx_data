...
class __Mustache_db1f4ee4989fc761e1a11bef56d9bb09 extends Mustache_Template
{
...
';
        ...
        // 'disabled' section
        $value = $context->find('disabled');
        $buffer .= $this->section4294895afea3e1ebb3e0936487d4bdc7($context, $indent, $value);
    ...
    private function section4294895afea3e1ebb3e0936487d4bdc7(Mustache_Context $context, $indent, $value)
    {
        $buffer = '';
    
        if (!is_string($value) && is_callable($value)) {
            $source = '
    <span class="currentlink {{classes}}" {{#attributes}} {{name}}={{#quote}}{{value}}{{/quote}} {{/attributes}}>{{#icon}}{{#context}}{{> &&template}}{{/context}}{{/icon}}{{{text}}}</span>
';
            $result = call_user_func($value, $source, $this->lambdaHelper);
            if (strpos($result, '{{') === false) {
                $buffer .= $result;
